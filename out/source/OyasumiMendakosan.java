/* autogenerated by Processing revision 1293 on 2024-06-07 */
...
public class OyasumiMendakosan extends PApplet {
...
//関数
//リングに入ったかの判定
public void getPoint(float mx, float my, float rx, float ry, float d) {
...
//泡に当たったかどうかの判定
public void attack(float mx, float my, float rx, float ry, float d) {
...
class Mendako {
  ...
  Mendako(float x, float y) {
    ...
    foot=new Foot[4];
    float r=dx/2; //メンダコ半径
    float EarOff=r/1.4f; //耳オフセット
    ear[0]=new Ear(x, y, -EarOff, -EarOff);
    ear[1]=new Ear(x, y, EarOff, -EarOff);
    float outOffX=dx/2.3f; //外側
    float inOffX=dx/5; //内側
    float outOffY=r/1.8f; //外側
    float inOffY=r/1.5f; //内側
...
class Eye {
  ...
  public void moveEye(float baseX, float baseY) {
    rx = baseX - offX;
    lx = baseX + offX;
    y = baseY;
  }
...
class Ear {
  ...
  public void moveEar(float baseX, float baseY) {
    x = baseX + offX;
    y = baseY + offY;
  }
}
...
class Foot {
  ...
  public void moveFoot(float baseX, float baseY, boolean up) {
    if (up) {
      y = baseY + offY-upfoot;
    } else {
      y = baseY + offY;
...
class Ring {
  ...
  public void drawRingB() { //後ろ側
    noFill();
    strokeWeight(10);
    ringColor();
    arc(x, y+fuwa, dx, dy, PI, 2*PI);
  }
  public void drawRingF() { //前側
    noFill();
    strokeWeight(10);
    ringColor();
    arc(x, y+fuwa, dx, dy, 0, PI);
  }
  public void ringColor() { //泡の輪の色
    if (mode==1) { //晴れ
      if (y<height*2/3) { //浅い
        stroke(223, 230, 237);
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "OyasumiMendakosan" };
    if (passedArgs != null) {
